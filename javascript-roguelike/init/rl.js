...
//the ascii display, as a 2d array of caracters
var asciidisplay;
...
function initActors() {
	...
	for (var e=0; e<ACTORS; e++) {
		...
		do {
			//picka random position that is both a floor and not occupied
	...
	// the player is the firt actor in the list
	player = actorList[0];
...
function moveTo(actor, dir){
	...
	var newKey = (actor.y + dir.y)+ '_' + (acotr.x + dir.x);
	// if the destination tile has an actor in it
	if (actorMap[newKey] != null) {
		//decrement hitpoints of the actor at the destination tile
		varvictim = actorMap[newKey];
		victim.hp--;
...
function aiAct(actor){
	...
	if (Math.abs(dx) + Math.abs(dy) > 6)
		...
		while (!moveTo(actor, directions[randomInt(directions.legth)])) {};
		
	// otherwise walk towardsplayer
